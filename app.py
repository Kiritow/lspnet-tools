...
def config_up(parser: NetworkConfigParser):
    ...
    if parser.enable_local_network:
        ...
        sudo_call(["iptables", "-t", "nat", "-I", "{}-POSTROUTING".format(parser.namespace), "-s", parser.local_network, "!", "-d", "224.0.0.0/4", "-o", "{}0".format(parser.local_veth_prefix) , "-j", "SNAT", "--to", get_eth_ip(parser.local_ethname)])
